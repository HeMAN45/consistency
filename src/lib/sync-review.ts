...
async function loadSlots(syncId: string, from: DayKey, to: DayKey) {
  ...
  const done = new Set(logs.map((l) => `${dateToDayKey(l.date)}:${l.syncTaskId}:${l.userId}`));
  const resting = new Set(rests.map((r) => `${dateToDayKey(r.date)}:${r.userId}`));
    ...
    for (const task of live) {
      for (const member of members) {
        ...
        slots.push({
          ...
          done: done.has(`${key}:${task.id}:${member.userId}`),
        });
      }
    }
  }
...
function ratio(slots: Slot[]) {
  ...
  return slots.filter((s) => s.done).length / slots.length;
}
...
export type SyncReview = {
  ...
  mostImproved: { displayName: string; deltaPoints: number } | null;
  members: { userId: string; displayName: string; done: number; total: number; pct: number }[];
  you: { thisWeek: number; trailing: number; deltaPoints: number; hasHistory: boolean };
};
...
export async function weeklySyncReview(
  ...
  const { slots, members, tasks } = await loadSlots(syncId, from, today);
  if (slots.length === 0) return null;

  const weekStart = shiftDayKey(today, -6);
  const previousStart = shiftDayKey(today, -13);

  const week = slots.filter((s) => s.key >= weekStart);
  const previous = slots.filter((s) => s.key >= previousStart && s.key < weekStart);
  ...
  let bestDay: { date: DayKey; ratio: number } | null = null;

  for (const [key, daySlots] of byDay) {
    const dayRatio = ratio(daySlots);
    if (dayRatio >= GROUP_STREAK_THRESHOLD) qualifyingDays += 1;
    if (!bestDay || dayRatio > bestDay.ratio) bestDay = { date: key, ratio: dayRatio };
  }

  // Weakest shared task this week.
  let weakestTask: { name: string; ratio: number } | null = null;
  for (const task of tasks) {
    const taskSlots = week.filter((s) => s.taskId === task.id);
    if (taskSlots.length === 0) continue;
    const taskRatio = ratio(taskSlots);
    if (!weakestTask || taskRatio < weakestTask.ratio) {
      weakestTask = { name: task.name, ratio: taskRatio };
    }
  }

  // Per member, plus who moved the most week on week.
  const memberRows = members.map((member) => {
    const mine = week.filter((s) => s.userId === member.userId);
  ...
  let mostImproved: { displayName: string; deltaPoints: number } | null = null;
  for (const member of members) {
    const now = ratio(week.filter((s) => s.userId === member.userId));
  ...
  // Your own comparison, against your own SYNC history. Nobody else sees it.
  const yourWeek = ratio(week.filter((s) => s.userId === userId));
  const yourTrailing = ratio(slots.filter((s) => s.userId === userId && s.key < weekStart));
  const yourHistory = slots.filter((s) => s.userId === userId && s.key < weekStart).length > 0;

  return {
    thisWeek: Math.round(thisWeek * 100),
    lastWeek: Math.round(lastWeek * 100),
    deltaPoints: Math.round((thisWeek - lastWeek) * 100),
    qualifyingDays,
    bestDay,
    weakestTask,
    mostImproved,
    members: memberRows.sort((a, b) => b.pct - a.pct),
    you: {
      thisWeek: Math.round(yourWeek * 100),
      trailing: Math.round(yourTrailing * 100),
      deltaPoints: Math.round((yourWeek - yourTrailing) * 100),
...
export async function seasonSummary(syncId: string, timezone: string) {
  const sync = await db.sync.findUnique({
    where: { id: syncId },
    select: { startDate: true, endDate: true },
  });
  if (!sync?.startDate || !sync.endDate) return null;

  const from = dateToDayKey(sync.startDate);
  const to = dateToDayKey(sync.endDate);
  const capped = to > todayKey(timezone) ? todayKey(timezone) : to;

  const { slots, members } = await loadSlots(syncId, from, capped);
  ...
  return { from, to, standings, totalSlots: slots.length };
}
